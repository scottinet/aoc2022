...
function compare(left = this.left, right = this.right, depth = 0) {
    for (let i = 0; i < Math.max(left.length, right.length); i++) {
        const leftValue = left[i];
        const rightValue = right[i];
        if (leftValue !== undefined && rightValue === undefined) {
            toolbox.debugLog(`More items available in the left part than in the right part at depth ${depth}`);
            return 1;
        }
        if (leftValue === undefined && rightValue !== undefined) {
            toolbox.debugLog(`More items available in the right part than in the left part at depth ${depth}`);
            return -1;
        }
...
function part1(input) {
    ...
    for (let i = 0; i < pairs.length; i++) {
        ...
        toolbox.debugLog(`Pair ${i + 1} is ${ordered <= 0 ? "" : "not "}ordered\n========`);
...
function part2(input) {
    ...
    toolbox.debugLog("The sorted signals are:\n" +
        signals.map((s) => `${JSON.stringify(s)}\n`).join(""));
    let decoderKey = 1;
    for (const key of dividers) {
        toolbox.debugLog(`The index of ${JSON.stringify(key)} is ${signals.indexOf(key)}`);
